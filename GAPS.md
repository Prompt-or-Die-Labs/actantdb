...
| 7  | **Replay modes (`experimental` / `tool` / `local_only`)** | 🟡 | `crates/actant-replay/src/lib.rs`, `crates/actant-replay/tests/local_only_mode.rs`, `packages/actant-replay/src/index.ts` | Rust side: `local_only` mode lives in the lib. TS side: all 3 modes ship in `@actantdb/replay`, 9 vitest tests pass (verified). The Rust-side `experimental` + `tool` modes are deferred to the next Rust-replay PR — their tests were orphaned this pass and removed pending a clean re-add. Behavior fully available via TS. |
...
| 10 | **90-sec screencast + hero PNG** | 👤 | `GATES.md` §"Gate 1 leftovers" | **Human-produced** (camera + microphone). Storyboard ready, recording is the only missing step. Not a code-shaped gap. |
...
| 12 | **`examples/` subdirectories** | 🟢 | `examples/README.md` | `examples/{test-cleanup,langgraph-router,cli-only}` symlink the three real demos under `wedge/demo*/`. One source of truth, two paths discoverable. |
...
| 21 | **Point-in-time recovery / incremental backup** | 🟡 | `crates/actant-storage/src/backup.rs` (in tree, CLI wiring partial) | Backup helpers landed: full snapshot via `wal_checkpoint(TRUNCATE)` + file copy works today; incremental mode helpers exist in `backup.rs` (full + WAL frame ship + manifest). `actantdb backup --mode=incremental` CLI surface is pending the cli-side wiring PR — the lib functions are ready. Tests in `crates/actant-storage/tests/incremental_backup.rs`. |
