...
| 41 | **XCFramework build + Swift binaryTarget** | 🟡 | This pass — Swift side. `.github/workflows/ios-xcframework.yml` ships: matrix builds the 5 iOS/macOS slices via `cargo build --release --target … -p actant-ffi --features ios`, lipos sim+mac universals, generates Swift bindings via `cargo run --bin uniffi-bindgen --features=cli`, runs `xcodebuild -create-xcframework`, zips + sha256s, uploads as release asset on tag push and as workflow_dispatch artifact. `sdks/swift/Package.swift` carries the `.binaryTarget(url:checksum:)` stanza as a commented placeholder with TODO + URL/checksum patterns documented so it's a one-line uncomment after the first `v0.0.X` tag ships. Won't run green until #40 lands the `ios` feature flag + the parallel `actant-ffi` agent exposes a `uniffi-bindgen` binary with a `cli` feature. |
...
| 46 | **Swift `Actant.embedded(storeDir:)` mode** | 🟡 | This pass. New `Sources/ActantDB/Actant.swift` (`actor Actant`) + `Mode.swift` + `Embedded/{EmbeddedHandle,ActantFFIBridge}.swift`. Construction modes: `.remote(URL, workspaceID:, actorID:)`, `.spawned(SpawnedSupervisor, dbPath:, …)` (host-only), `.embedded(storeDir:, workspaceID:, actorID:)`. Unified API: `dispatch(command:input:idempotencyKey:) -> CommandOutcome`, `eventsSince(_:limit:) -> [EventRow]`, `ingest(_:) -> IngestReport`. `ActantFFIBridge` is `#if canImport(ActantFFI)`-gated; the fallback throws `ActantError.transport("ActantFFI binary target not linked …")` so `Actant.embedded(...)` fails fast until #41's first XCFramework release lands. `ActantDBSupervisor` now conforms to a new `SpawnedSupervisor` protocol seam to keep the dependency direction one-way. Tests in `ActantModeTests.swift` cover remote dispatch, remote eventsSince adaptation, remote-ingest unsupported error, spawned-mode construction via a stub supervisor (host-only), and the FFI-missing failure path. Embedded round-trip is `#if canImport(ActantFFI)`-gated — runs only after the binary target is wired. |
...
**Part C sub-totals:** 9 🟢, 3 🟡 (Swift-side #41/#46/#47 landed; await `actant-ffi` crate + first XCFramework release tag), 1 🔴 (#40 iOS-clean audit + feature gates).
