...
| 5  | **Postgres storage + command-engine** | 🟢 | `crates/actant-storage/src/{postgres,pg_repo,backend}.rs`, `crates/actant-command/src/lib.rs`, `migrations/pg/` | `PgStorage` has repo-method parity for the command substrate and `Engine::postgres` dispatches through backend-neutral methods. The HTTP server is explicitly SQLite-only until its raw route SQL is ported. |
...
| 15 | **`ACTANTDB_DATABASE_URL` silent-ignore** | 🟢 | `crates/actant-server/src/bin/server.rs` | Server refuses to start with the env var because the HTTP route layer is still SQLite-only. This prevents silent downgrade while row #5 remains available to direct Rust storage/command-engine users. |
...
| 33 | **Connection pooler in front of Postgres** | 🛣 | row #5 closure (PG storage + command-engine) | Supabase ships Supavisor (their pooler). When ActantDB Cloud's Phase 2 hosted backend lands, the control plane needs PgBouncer / Supavisor in the deployment recipe. Direct Rust users of `PgStorage` can add their own pooler today; the HTTP server does not yet accept `ACTANTDB_DATABASE_URL`. |
...
| 41 | **XCFramework build + Swift binaryTarget** | 🟡 | `.github/workflows/ios-xcframework.yml` matrix-builds the 5 iOS/macOS slices via `cargo build --release --target … -p actant-ffi --features ios`, lipos sim+mac universals, generates Swift bindings from the host cdylib with `cargo run --bin uniffi-bindgen -p actant-ffi`, diffs the generated Swift against committed `sdks/swift/Sources/ActantFFI/actant_ffi.swift`, runs `xcodebuild -create-xcframework`, zips, computes the SwiftPM checksum with `swift package compute-checksum`, and uploads release/artifact assets. `sdks/swift/Package.swift` now has the real target shape: local/released `actant_ffiFFI` binary target + `ActantFFI` generated Swift source target + `ACTANTDB_FFI` define for `ActantDB`. It remains 🟡 until the first release asset exists and `releasedActantFFI` is pinned to its URL/checksum. Local rebuild was blocked on 2026-05-22 by disk exhaustion before Cargo finished release compilation; see `TESTING.md`. |
...
| 47 | **CloudKit sync target (`Sources/ActantDB/Sync/CloudKit/`)** | 🟡 | This pass. New `Sources/ActantDB/Sync/{SyncTypes,None/NoOpSync,CloudKit/{CloudKitSync,CKRecordMapping,SubscriptionManager,OutboxDrainer}}.swift`. Cross-platform surface: `ActantSync` protocol + `SyncOptions(retainDays:pushOnAppActive:)` + `SyncStatus(syncedAtHLC:outboxQueueDepth:lastInboundEventAt:activeSubscription:)` + `SyncError`. CloudKit side: per-workspace zone (`ActantDBLedger`), one `CKRecord(recordType:"ActantEvent")` per `EventRow`, `CKQuerySubscription(firesOnRecordCreation)` for resume, exponential backoff drainer modeled on `actant-reliability::circuit`. Outbox is currently in-memory; durable `cloudkit_outbox` persistence waits on substrate (TODO marked in `OutboxDrainer.swift`). `NoOpSync` ships on non-Apple. `Actant.sync` returns a `DeferredCloudKitSync` wrapper on Apple, `NoOpSync` elsewhere. Tests: `CKRecordMappingTests.swift` (3 round-trip tests, Apple-only) + `NoOpSyncTests.swift` (3 unsupported-platform asserts, non-Apple-only). |
...
**Part C sub-totals:** 11 🟢, 2 🟡 (#41, #47 — the Swift embedded mode is wired
for local ActantFFI artifacts; the published binaryTarget still waits for the
first `vX.Y.Z` XCFramework release), 0 🔴.
