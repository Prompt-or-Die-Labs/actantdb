...
iOS/sync rows = 48 repo-verifiable rows.** Workspace at **31 crates** under
`crates/` and 33 Cargo workspace packages. Package manifests are at `0.0.15`
locally.
...
| 5  | **Postgres storage + command-engine + core HTTP mode** | 🟢 | `crates/actant-storage/src/{postgres,pg_repo,backend}.rs`, `crates/actant-command/src/lib.rs`, `crates/actant-server/src/lib.rs`, `migrations/pg/` | `PgStorage` has repo-method parity for the command substrate, `Engine::postgres` dispatches through backend-neutral methods, and `actantdb-server` boots from `ACTANTDB_DATABASE_URL` for the core HTTP surface: health, metadata, typed commands, events, and sync pulls. SQLite-only Studio/admin routes return explicit 501 in Postgres mode. |
...
| 15 | **`ACTANTDB_DATABASE_URL` silent-ignore** | 🟢 | `crates/actant-server/src/bin/server.rs` | Server now treats `ACTANTDB_DATABASE_URL` as Postgres mode and never silently falls back to SQLite. Unsupported SQLite-only routes return explicit 501 JSON. |
...
| 22 | **Postgres schema-parity CI gate** | 🟢 | `.github/workflows/ci.yml` `migrations-parity` job | Textual `CREATE TABLE` diff currently reports 91 SQLite tables and 91 Postgres tables; hard-fails on PG-only tables and logs SQLite-only gaps. |
| 23 | **Empty `sdks/rust/` target dir** | 🟢 | row #2 closure | Resolved with #2; `target/` moved to `~/.cache/cargo-actantdb`. |
| 24 | **PG migration runtime apply order** | 🟢 | `crates/actant-storage/src/postgres.rs` `PG_MIGRATIONS` | All seven `migrations/pg/*.sql` files are registered in dependency-correct order (`0001 → 0005 → 0002 → 0003 → 0004 → 0006 → 0007`). |
...
| 26 | **`docker-compose.yml` for one-line self-host** | 🟢 | `deploy/docker-compose.yml`, `deploy/Dockerfile`, `deploy/README.md`, `deploy/caddy/Caddyfile` | One-line self-host: SQLite-backed `actantdb-server` + Caddy (auto-TLS via `ACTANTDB_DOMAIN`) + Mailpit (SMTP catcher, closes DEVX_GAPS X94). `docker compose -f deploy/docker-compose.yml up` is enough. |
...
| 28 | **`actantdb dev` watch loop** | 🟢 | `crates/actant-cli/src/cmd/watch_dev.rs` | `notify::RecommendedWatcher` (poll fallback at 500 ms) bridged to tokio via `tokio::sync::mpsc::unbounded_channel`. Watches `commands/`, `policies/`, `templates/`, `crates/actant-contracts/src/` by default (override with `--watch-dirs`). Re-validates `*.actant.json` / `policy.json` as `actant_policy::PolicyDoc` and re-runs `cargo run -p actant-contracts --bin actant-contracts -- codegen-ts` on contract changes. |
...
| 31 | **Auto-generated TS types from schema/contracts (live, on-save)** | 🟢 | `crates/actant-contracts/`, `packages/actant-types/src/generated/` | `cargo run -p actant-contracts --bin actant-contracts -- codegen-ts` writes generated TS to `packages/actant-types/src/generated/`. Same effective shape as Supabase's `supabase gen types typescript` or Convex's `_generated/`. Missing the `--watch` flavor (would tie into row #28's `actantdb dev`). |
| 32 | **Built-in OAuth/OIDC provider chain (Google/GitHub/Apple/etc.)** | 🟢 | `crates/actant-auth/src/oidc.rs`, `crates/actant-server/src/lib.rs`, `crates/actant-server/assets/login.html` | OIDC token verification, linking-code auth, argon2id password login, and configurable provider buttons are in tree. Set `ACTANTDB_OAUTH_PROVIDERS=google,github`, `ACTANTDB_OAUTH_GOOGLE_AUTH_URL`, `ACTANTDB_OAUTH_GOOGLE_CLIENT_ID`, and `ACTANTDB_PUBLIC_BASE_URL`; `/login` renders provider buttons and `/v1/auth/oauth/{provider}/start` builds the authorization redirect. Provider callback token exchange remains intentionally outside the self-hosted binary unless a host supplies client-secret storage. |
| 33 | **Connection pooler in front of Postgres** | 🛣 | row #5 closure (PG storage + command-engine + core HTTP mode) | Supabase ships Supavisor (their pooler). ActantDB self-host uses `sqlx::PgPool` directly in `PgStorage` and `actantdb-server` now accepts `ACTANTDB_DATABASE_URL`; a PgBouncer / Supavisor sidecar remains a hosted-control-plane deployment concern, not an in-process backend gap. |
| 34 | **Hosted log streaming / observability surface** | 🛣 | `actant-core::trace` (trace/span IDs) | Supabase Cloud ships Logflare + Vector + log drains. ActantDB ships trace/span ID primitives and OTel/OpenInference schema columns; the exporter/control-plane log UI is still a hosted/ops surface. Self-host today: bring your own collector and wire export at the app/server boundary. |
...
2. Keep the Studio first-run path tested so empty ledgers point users at a
...
| 41 | **XCFramework build + Swift binaryTarget** | 🟢 | `.github/workflows/ios-xcframework.yml` matrix-builds the 5 iOS/macOS slices via `cargo build --release --target … -p actant-ffi --features ios`, lipos sim+mac universals, generates Swift bindings from the host cdylib with `cargo run --bin uniffi-bindgen -p actant-ffi`, diffs the generated Swift against committed `sdks/swift/Sources/ActantFFI/actant_ffi.swift`, runs `xcodebuild -create-xcframework`, zips, computes the SwiftPM checksum with `swift package compute-checksum`, and uploads release/artifact assets. `sdks/swift/Package.swift` wires both local and release binary targets: `ACTANTDB_LOCAL_FFI_XCFRAMEWORK` uses a local path; `ACTANTDB_FFI_XCFRAMEWORK_CHECKSUM` plus optional `ACTANTDB_FFI_XCFRAMEWORK_URL` or `ACTANTDB_FFI_RELEASE_TAG` activates the remote SwiftPM `.binaryTarget(url:checksum:)` without editing the manifest. |
...
| 46 | **Swift `Actant.embedded(storeDir:)` mode** | 🟢 | `Sources/ActantDB/Actant.swift` (`actor Actant`) + `Mode.swift` + `Embedded/{EmbeddedHandle,ActantFFIBridge}.swift`. Construction modes: `.remote(URL, workspaceID:, actorID:)`, `.spawned(SpawnedSupervisor, dbPath:, …)` (host-only), `.embedded(storeDir:, workspaceID:, actorID:)`. Unified API: `dispatch(command:input:idempotencyKey:) -> CommandOutcome`, `eventsSince(_:limit:) -> [EventRow]`, `ingest(_:) -> IngestReport`. `ActantFFIBridge` maps the current uniffi `ActantHandle.open/dispatch/events_since/ingest/close` records field-by-field when the FFI binary target is enabled through `ACTANTDB_FFI`; the fallback still throws `ActantError.transport("ActantFFI binary target not linked …")` when no artifact is present. Tests in `ActantModeTests.swift` cover remote dispatch, remote eventsSince adaptation, remote-ingest unsupported error, spawned-mode construction via a stub supervisor (host-only), FFI-missing failure, and an `ACTANTDB_FFI` embedded dispatch + eventsSince round trip. |
| 47 | **CloudKit sync target (`Sources/ActantDB/Sync/CloudKit/`)** | 🟢 | This pass. `Sources/ActantDB/Sync/{SyncTypes,None/NoOpSync,CloudKit/{CloudKitSync,CKRecordMapping,SubscriptionManager,OutboxDrainer,CloudKitOutboxStore}}.swift`. Cross-platform surface: `ActantSync` protocol + `SyncOptions(retainDays:pushOnAppActive:)` + `SyncStatus(syncedAtHLC:outboxQueueDepth:lastInboundEventAt:activeSubscription:)` + `SyncError`. CloudKit side: per-workspace zone (`ActantDBLedger`), one `CKRecord(recordType:"ActantEvent")` per `EventRow`, `CKQuerySubscription(firesOnRecordCreation)` for resume, exponential backoff drainer modeled on `actant-reliability::circuit`. `CloudKitOutboxStore` persists queued events to Application Support as JSON, dedupes by event id, and removes only confirmed uploads. `NoOpSync` ships on non-Apple. Tests: `CKRecordMappingTests.swift` (3 round-trip tests, Apple-only), `CloudKitOutboxStoreTests.swift` (3 durable queue tests), and `NoOpSyncTests.swift` (3 unsupported-platform asserts, non-Apple-only). |
