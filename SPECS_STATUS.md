...
| 09 — SDK design | **verified** | `crates/actant-contracts/tests/spec_09_verification.rs` | 4 tests: TS+Python SDKs expose every alpha command, brace-matched extraction of `command()` body proves no silent retry. |
...
| 14 — Extended primitives | **verified** | `crates/actant-policy/tests/spec_14_verification.rs` | 3 tests: every table in 0002, cross-cutting columns in spec 02, every primitive referenced from spec 13. |
| 15 — ActantIndex | **verified** | `crates/actant-memory/tests/spec_15_verification.rs` | 2 tests: every index table in 0003, dense path + VectorStore + InMemoryStore present. |
| 16 — Protocols | **verified** | `crates/actant-core/tests/spec_16_verification.rs` | 3 tests: MCP+A2A+AP2 tables present, AP2Mandate enforces spend limit, ActantA2aCard type usable. |
| 17 — Observability | **verified** | `crates/actant-core/tests/spec_17_verification.rs` | 3 tests: trace+span ids match W3C, single redaction chokepoint, otel columns in schema. |
| 18 — Reliability primitives | **verified** | `crates/actant-reliability/tests/spec_18_verification.rs` | 3 tests: every reliability table in 0003, token-bucket invariant, circuit-state transitions. |
| 19 — Performance architecture | **verified** | `crates/actant-command/tests/spec_19_verification.rs` | 3 tests: no external I/O in hot-path crates, bench harness exists, kernel dispatch covers alpha commands. |
...
| 0005 — Data capsules | `adr_0005_data_capsules_have_table_and_type` | `capsule` table + `ActantCapsule` Rust type exist. |
| 0007 — Behavioral trust | `adr_0007_behavioral_trust_has_score_confidence_samples` | `ActantTrustProfile` carries score + confidence + sample_size. |
...
| 0018 — Hot kernel | `adr_0018_hot_kernel_exists` | dispatch_tool_call + ActantHotToolCall public API. |
| 0020 — Deployment modes | `adr_0020_deployment_modes_have_helm_chart` | Helm chart + Dockerfile present. |

ADRs without a dedicated ADR verifier test today (and why):
...
- **0012 — Hybrid retrieval**: in-memory dense + sparse + graph search exists in `actant-memory::index` and is covered by spec 15 verification; external vector backends remain adapter work.
- **0013 — Rerank default**: local deterministic rerank exists in `actant-memory::index` and is covered by spec 15 verification; neural/cloud rerankers remain adapter work.
