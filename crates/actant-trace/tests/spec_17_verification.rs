...
fn trace_and_span_ids_match_w3c_format() {
    use actant_trace::{new_span_id, new_trace_id};
    let t = new_trace_id();
    assert_eq!(t.len(), 32);
...
fn redaction_is_a_single_chokepoint() {
    ...
    // actant-trace."
    let lib = read_repo("crates/actant-trace/src/lib.rs");
    // We allow this to be implemented; we just confirm there is exactly
    // one trace_id minter and one span_id minter rather than scattered.
