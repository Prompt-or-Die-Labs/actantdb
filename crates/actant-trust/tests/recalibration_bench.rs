...
//! `TrustProfile::observe()` primitive — there is no service-level
//! recalibration pass. We bench the closest moral equivalent: for each of
//! 10k synthetic actors, construct a `TrustProfile`, feed a moderate
...
fn recalibration_of_10k_actors_under_5s() {
    ...
    for actor_idx in 0..ACTORS {
        let mut p = TrustProfile::new(format!("actor:{actor_idx}:tool.shell.run"));
