...
//! policy crate exposes the
//! `ActantTrustProfile { new, observe, auto_approve_at }` API:
//! - cold-start `ActantTrustProfile::new(area)` returns `score=0.5, confidence=0.0,
...
fn cold_start_sample_size_is_zero_and_confidence_is_zero() {
    let p = ActantTrustProfile::new("tool:test.cold");
...
fn nan_outcome_loop_does_not_panic() {
    ...
    let mut p = ActantTrustProfile::new("tool:test.alternating");
    for i in 0..10_000 {
        p.observe(i % 2 == 0);
...
fn all_failures_drives_score_to_zero_without_panic() {
    let mut p = ActantTrustProfile::new("tool:test.fail");
    for _ in 0..1000 {
        p.observe(false);
    }
...
fn all_successes_drives_score_to_one_without_panic() {
    let mut p = ActantTrustProfile::new("tool:test.win");
    for _ in 0..1000 {
        p.observe(true);
    }
    assert!(p.score.is_finite());
