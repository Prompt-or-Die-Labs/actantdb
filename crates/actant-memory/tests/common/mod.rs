...
use actant_memory::{ActantIndex, ActantIndexedItem};
...
pub async fn build_index(fixture: &RecallFixture) -> ActantIndex {
    ...
    for q in &fixture.queries {
        for gold_id in &q.gold_doc_ids {
            idx.insert(ActantIndexedItem {
                id: gold_id.clone(),
                text: q.query.clone(),
                embedding: embedder_embed(&embedder, &q.query).await,
            });
        }
    }
    idx
...
pub async fn recall_at_k(fixture: &RecallFixture, idx: &ActantIndex, k: usize) -> f64 {
    let embedder = HashEmbedder::new();
    let mut hits = 0usize;
