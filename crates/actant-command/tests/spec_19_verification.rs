...
fn no_external_io_inside_transactions() {
    ...
    for crate_dir in [
        ...
        "crates/actant-command/src/kernel.rs",
    ] {
...
async fn kernel_dispatch_table_covers_alpha_commands() {
    // Spec 19: "the hot command dispatch table covers every alpha command."
    // Approximation: the kernel can dispatch a tool call through the engine.
    use actant_command::Engine;
    use actant_command::{dispatch_tool_call, ActantHotToolCall};
    use actant_core::*;
