...
use actant_workers::file::FileHandler;
use clap::Parser;

#[derive(Debug, Parser)]
#[command(name = "actant-worker-file", version)]
struct Args {
    #[arg(long, env = "ACTANTDB_DB")]
    db: PathBuf,
    #[arg(long, env = "ACTANTDB_WORKSPACE")]
    workspace: String,
    #[arg(long, default_value = "act_worker_file")]
    actor: String,
    #[arg(long, default_value = "file-worker-01")]
    name: String,
}
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut runner = WorkerRunner::new(queue, desc, vec![Box::new(FileHandler)], rx);
    runner.run().await?;
    Ok(())
}
