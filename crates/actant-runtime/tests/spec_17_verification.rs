...
fn trace_and_span_ids_match_w3c_format() {
    use actant_runtime::trace::{new_span_id, new_trace_id};
    let t = new_trace_id();
    assert_eq!(t.len(), 32);
...
fn redaction_is_a_single_chokepoint() {
    ...
    // actant-trace." After the actant-runtime merge the chokepoint moved to
    // crates/actant-runtime/src/trace.rs; assert there is still exactly one
    // trace_id minter and one span_id minter, rather than scattered.
    let lib = read_repo("crates/actant-runtime/src/trace.rs");
    let trace_count = lib.matches("pub fn new_trace_id").count();
    let span_count = lib.matches("pub fn new_span_id").count();
