...
//!
//! Two operations land in Phase 1 with a deterministic in-memory emulator:
//!
//!   - `browser.navigate`  — visit a URL, returns the title.
//!   - `browser.click`     — click a CSS selector.
//!   - `browser.type`      — type text into a field.
//!   - `browser.screenshot`— capture the page (hash only).
//!
//! Real Chromium driver (WebDriver / CDP) is Phase 2 — there's a `Driver`
//! trait so swapping in `chromiumoxide` or `fantoccini` is one file. The
//! shipped in-memory emulator records every action so tests can assert
//! exact sequences.
