//! Spec — `actant-prompts` registry round-trip.
//!
//! The work-package's first "Tests" bullet is
//! "Round-trip: create + add_version + retrieve by `(name, version)`". The
//! v0.1 in-memory `Template` exposes that round-trip directly: construct a
//! template, push one or more `Version`s, and retrieve by `(name, version)`
//! via `Template::render`. This file pins that behaviour so future moves to
//! a Storage-backed `PromptService` (per the spec) preserve the contract.

use actant_prompts::{Template, Version};
