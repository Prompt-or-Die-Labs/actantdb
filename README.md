...
| -------------------- | ------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| **Wedge (npm)**      | 8 TypeScript packages: `@actantdb/mastra`, `core`, `policy`, `replay`, `studio`, `types`, `sdk`, `convex`. ESM, Node ≥22.5. | `packages/*`, `dist-publish/*.tgz` install-verified |
...
- **Gate 2 — external adoption** (target 2026-07-31): blocked on `npm publish` + outreach. Tarballs in [`dist-publish/`](./dist-publish/) are install-verified into a clean sandbox.
...
├── dist-publish/                   — install-verified npm tarballs
