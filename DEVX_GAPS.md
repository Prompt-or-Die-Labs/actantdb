...
| X21 | **Ollama / local models** | 🟢 | `actant-command::models` registry knows about Ollama, the docs recipe points at `examples/ollama-only/`, and `pnpm --filter actant-demo-ollama-only demo:mock` records a local-model run plus a blocked cloud completion tool. |
...
|---|---|--------|-------|
| X52 | **OpenTelemetry exporter** | 🟡 | `actant-core::trace` ships trace/span ID primitives and the storage schema carries OTel columns. The actual exporter remains an app/server boundary integration, not a standalone runtime crate. |
...
| X90 | **Retrieval backend APIs** | 🟡 | `actant-memory::index` now ships backend-owned dense + sparse + graph + local-rerank retrieval in the concrete `ActantIndex` API. Remaining work is product surface, not substrate: Studio inspection panel, collection lifecycle, per-collection embedding model, and external vector backend adapters. This stays an agent-backend capability, not a standalone vector DB pivot. |
